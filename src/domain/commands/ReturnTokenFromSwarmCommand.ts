...
export class ReturnTokenFromSwarmCommand {
  ...
  constructor(targetWallet: string, wallets: WalletInfo[], tokenMint: string) {
  ...
  async execute(): Promise<void> {
    ...
    if (this.targetWallet === 'phantom') {
      ...
      throw new Error(
        'Phantom wallet integration not implemented for token transfers'
      );
    } else {
    ...
    for (const wallet of selectedWallets) {
      try {
        ...
        console.error(
          `Failed to transfer tokens from wallet ${wallet.publicKey}:`,
          error
        );
  ...
  private async executeTokenTransfer(
    ...
    const targetPublicKey = new PublicKey(
      this.targetWallet !== 'phantom' ? this.targetWallet : this.targetWallet
    );
    globalEventEmitter.emit<BalanceChangeData>(
